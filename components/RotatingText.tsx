import { FC, CSSProperties, forwardRef, useCallback, useEffect, useImperativeHandle, useMemo, useState } from "react"
import { motion, AnimatePresence, Transition, Target, TargetAndTransition } from "framer-motion"
...
export interface RotatingTextProps extends Omit<React.ComponentPropsWithoutRef<typeof motion.span>, "children" | "transition" | "initial" | "animate" | "exit"> {
  ...
  animate?: Target | TargetAndTransition;
  exit?: Target | string;
...
const RotatingText = forwardRef<RotatingTextRef, RotatingTextProps>(
  (
    ...
    // Split text into characters or words based on `splitBy`
    ...
    const elements = useMemo(() => {
      ...
      return currentText.split(splitBy).map((part) => ({
        characters: [part],
      }));
    }, [texts, currentTextIndex, splitBy]);

    // Get stagger delay based on "staggerFrom" prop
    ...
    // Automatic text change with interval
    useEffect(() => {
      if (!auto) return;
